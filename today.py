...
def simple_request(func_name, query, variables):
    ...
    if request.status_code == 200:
        return request
    raise Exception(
        f"{func_name} has failed with a {request.status_code}, {request.text}, QUERY_COUNT={QUERY_COUNT}"
    )
...
def recursive_loc(
    ...
    while True:
        ...
        if request.json()["data"]["repository"]["defaultBranchRef"] is None:
            return 0  # Only count commits if repo isn't empty
...
def loc_counter_one_repo(
    ...
    """
    Adds the LOC value of commits authored by me for one history page
...
def svg_overwrite(
    filename, commit_data, star_data, repo_data, contrib_data, stats_data, loc_data
):
    """
    ...
    tspan[36].firstChild.data = repo_data
    tspan[38].firstChild.data = contrib_data
    tspan[40].firstChild.data = star_data
    tspan[42].firstChild.data = commit_data
    tspan[44].firstChild.data = stats_data["issues"]
    tspan[46].firstChild.data = stats_data["prs"]
    tspan[48].firstChild.data = loc_data[2]
    tspan[49].firstChild.data = loc_data[0] + "++"
    tspan[50].firstChild.data = loc_data[1] + "--"
...
if __name__ == "__main__":
    ...
    # if OWNER_ID == {
    #     "id": "MDQ6VXNlcjYzMzQyNDY0"
    # }:  # only calculate for user ugly-custard

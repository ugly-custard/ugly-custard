"""
Original: Andrew Grant (Andrew6rant), 2022-2024
Modified by: ugly-custard, 2024
"""
...
# Account permissions:
#       read:Followers, read:Starring, read:Watching
# Repository permissions:
#       read:Contents, read:Commit statuses,
...
def simple_request(func_name, query, variables):
    ...
    if request.status_code == 200:
        return request
    raise Exception(f"{func_name} has failed with a {request.status_code}, {request.text}, QUERY_COUNT={QUERY_COUNT}")
...
def recursive_loc(
    ...
    addition_total=0, deletion_total=0,
    my_commits=0, cursor=None
):
    """
    ...
    raise Exception(f"recursive_loc() has failed with a {request.status_code}, {request.text}, QUERY_COUNT={QUERY_COUNT}")
...
def loc_counter_one_repo(
    ...
    if history['edges'] == [] or not history['pageInfo']['hasNextPage']:
        ...
        return recursive_loc(
            owner, repo_name, data, cache_comment, addition_total,
            deletion_total, my_commits, history['pageInfo']['endCursor']
        )


def loc_query(
    owner_affiliation, comment_size=0,
    force_cache=False, cursor=None, edges=[]
):
    """
...
def graph_repos_stars(
    count_type, owner_affiliation,
    cursor=None, add_loc=0, del_loc=0
):
    """
...
def stats_getter():
    """
    Returns the number of pr, mr, open and closed issues of the user
    """
...
def svg_overwrite(filename, commit_data, star_data, repo_data, contrib_data, stats_data, loc_data):
    """
    ...
    tspan[55].firstChild.data = repo_data
    tspan[57].firstChild.data = contrib_data
    tspan[59].firstChild.data = star_data
    tspan[61].firstChild.data = commit_data
    tspan[63].firstChild.data = stats_data['issues']
    tspan[65].firstChild.data = stats_data['prs']
    tspan[67].firstChild.data = loc_data[2]
    tspan[68].firstChild.data = loc_data[0] + '++'
    tspan[69].firstChild.data = loc_data[1] + '--'
...
if __name__ == "__main__":
    ...
    # if OWNER_ID == {'id': 'MDQ6VXNlcjYzMzQyNDY0'}: # only calculate for user Andrew6rant
    ...
    svg_overwrite('dark_mode.svg', commit_data, star_data, repo_data, contrib_data, stats_data, total_loc[:-1])
    svg_overwrite('light_mode.svg', commit_data, star_data, repo_data, contrib_data, stats_data, total_loc[:-1])

    # move cursor to override 'Calculation times:' with 'Total function time:' and the total function time, then move cursor back
    print('\033[F\033[F\033[F\033[F\033[F\033[F\033[F\033[F',
        '{:<21}'.format('Total function time:'), '{:>11}'.format('%.4f' % (user_time + loc_time + commit_time + star_time + repo_time + contrib_time + stats_time)),
